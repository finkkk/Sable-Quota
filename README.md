...
- 修复通过断裂一个大型结构批量生成碎片来绕过数量上限的问题。
- 玩家破坏关键连接方块前会进行连通性预判；超额时直接取消破坏，不再先断裂后补救。
- 创建成功显示白色 `已用/上限` Action Bar，额度不足显示红色 Action Bar，不占用聊天框。
- 新增 `/sablequota add` 与 `/sablequota remove` 管理指令。
- 优化断裂扫描、服务端翻译、SavedData 访问和延迟装配临时队列。
...
- 创建成功时在 Action Bar 以白字显示当前结构数量；配额拦截提示也显示在 Action Bar，不占用聊天框。
- 结构真正删除后自动释放额度；普通卸载不会误减计数。
- 玩家破坏方块会导致结构断裂并超过剩余名额时，在方块被破坏前取消操作；其他来源的自动拆分仍有配额兜底。
- Sable 自动拆分出的结构继承原结构所有者，每个拆分结构都会占用一个名额。
...
只需要将构建出的 JAR 放入服务端 `mods` 目录，客户端无需安装。配额判断、数据保存和文本翻译都由服务端完成；服务端会读取玩家上报的语言，中文客户端显示中文，其他语言目前回退到英文。客户端额外安装不会影响功能，但不是必需条件。
...
普通玩家可以查询自己；查询他人、设置、增减、重置和重载需要权限等级 2。
...
| `/sablequota add <player> <amount>` | 在当前有效上限上增加额度，并保存为个人上限 |
| `/sablequota remove <player> <amount>` | 减少当前有效上限，最低为 `0`，并保存为个人上限 |
| `/sablequota reset <player>` | 删除个人上限，恢复跟随普通玩家/OP 默认值 |
| `/sablequota reload` | 校验并重新读取 TOML；失败时旧配置继续生效 |

`add` 和 `remove` 的数量参数必须至少为 `1`。如果目标当前为无限上限，需要先使用 `set` 设置有限值。
...
玩家破坏结构方块时，只有在局部邻居可能被分成多个连通分量且剩余额度不足的情况下才进行图遍历。遍历使用 Sable 相同的 18 邻接规则、原始 long 坐标集合与提前终止策略，不会每 Tick 扫描结构。
